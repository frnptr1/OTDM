...
iout_bls = 0;
%
fprintf('[otdm_uo_solve]     k  f(xk)            ||gk||        min(lak)  alk           w1        w2        g1        g2\n');
while norm(g) > eps && k < maxiter
    ...
    %
    if mod(k,outfreq) == 0
        fprintf('[otdm_uo_solve] %5d  %+10.8e  %8.6e  %+4.1e  %+4.1e    %7.3f   %7.3f   %7.3f   %7.3f    \n', k, f_fun(xk(:,k)), norm(gk(:,k)), la(1), al, x(1), x(2), gk(:,k));
    end
    if iout_bls == 1
